<?php
/**
 * Fallback template for archives, search, and latest posts.
 *
...
<section class="lsg-hero lsg-blog-hero">
  <div class="lsg-container">
    <div class="lsg-blog-hero__inner">
      <div>
        <span class="lsg-eyebrow">
          <?php echo is_search() ? esc_html__( 'Search', 'luxury-spa-gulshan' ) : esc_html__( 'Spa Journal', 'luxury-spa-gulshan' ); ?>
        </span>
        <h1 class="lsg-heading lsg-heading--hero">
          ...
          if ( is_archive() ) {
              ...
              printf( esc_html__( 'Search Results for: %s', 'luxury-spa-gulshan' ), esc_html( get_search_query() ) );
          } else {
              esc_html_e( 'Latest Articles', 'luxury-spa-gulshan' );
          }
          ?>
        </h1>
        <?php if ( is_archive() && get_the_archive_description() ) : ?>
        <p class="lsg-lead"><?php echo wp_kses_post( get_the_archive_description() ); ?></p>
        <?php else : ?>
        <p class="lsg-lead"><?php esc_html_e( 'Browse wellness tips, spa treatment guides, and updates from Luxury Spa Gulshan.', 'luxury-spa-gulshan' ); ?></p>
        <?php endif; ?>
      </div>
...
<section class="lsg-section">
  <div class="lsg-container">
    ...
    <article class="lsg-prose">
      <p><?php esc_html_e( 'Nothing found. Try a different search or browse our services.', 'luxury-spa-gulshan' ); ?></p>
    </article>
    <?php endif; ?>
  </div>
</section>
